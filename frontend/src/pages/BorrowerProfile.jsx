...
const CSS = `
  ...
  .strategy-btn.active { background:#6366f1; color:white; border-color:#6366f1; }
...
export default function BorrowerProfile() {
  ...
  const handlePayRow = async (ins) => {
    ...
    const strategy = inputs.strategy || 'BAL' // 'BAL' or 'COVER'
    ...
    try {
      if (strategy === 'BAL') {
        ...
        // Multi-Month Coverage Logic
        await api.patch(`/installments/${ins.id}/pay`, {
          paid_date: paymentDate,
          amount_collected: paidAmt,
          strategy: 'AUTO_SPLIT', // Tell backend to split automatically
          penalty: totalFine, 
          method: paymentMethod,
          receipt_no: receiptNo,
          cheque_no: chequeNo,
              ...
              <table className="lt">
                ...
                <tbody>
                  {installments.map((ins, idx) => {
                    ...
                    const inputs = rowInputs[ins.id] || {}
                    const paidAmt = Number(inputs.paid_amount || ins.amount_due)
                    const emi = Number(ins.amount_due)
                        ...
                        <tr style={isFirstPending ? { background: '#eff6ff' } : {}}>
                          ...
                          {ins.status !== 'PAID' ? (
                            <>
                              ...
                              <td>
                                <input type="number" className="fi mono green" style={{ fontSize: 13 }} value={inputs.paid_amount !== undefined ? inputs.paid_amount : Number(ins.amount_due).toFixed(0)} onChange={e => updateRowInput(ins.id, 'paid_amount', e.target.value)} />
                                {paidAmt > emi && (
                                  <div className="breakup-chip">
                                    {strategy === 'COVER' ? `Covers ${coverageCount} Months` : `₹${paidAmt - emi} Adj in Balance`}
                                  </div>
                                )}
                              </td>
                              ...
                              <td className="mono" style={{ fontWeight: 700, color: '#0f172a' }}>₹{fmtCurrency((Number(ins.balance || 0) + Number(ins.amount_due)) + (lateDays * (inputs.fine_rate || 10)) - (ins.temp_paid_amount || Number(ins.amount_due)))}</td>
